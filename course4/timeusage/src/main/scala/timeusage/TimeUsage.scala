...
object TimeUsage {
  ...
  def dfSchema(columnNames: List[String]): StructType =
    StructType.apply(StructField(columnNames.head, StringType, false) :: columnNames.tail.map(StructField(_, DoubleType, false)))
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    def classifyColumns(columnNames: List[String], prefixes: List[String]): List[Column] =
      columnNames
        .filter(colName => prefixes.exists(prefix => colName.startsWith(prefix)))
        .map(col)

    val primaryColumns = classifyColumns(columnNames, List("t01", "t03", "t11", "t1801", "t1803"))
    val workingColumns = classifyColumns(columnNames, List("t05", "t1805"))
    val otherPrefixes = List("t02", "t04", "t06", "t07", "t08", "t09", "t10", "t12", "t13", "t14", "t15", "t16", "t18")
    (primaryColumns, workingColumns,
      classifyColumns(columnNames, otherPrefixes) diff primaryColumns diff workingColumns)
  }
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    summed.groupByKey(tur => (tur.working, tur.sex, tur.age))
      .agg(typed.avg(tur => tur.primaryNeeds), typed.avg(tur => tur.work), typed.avg(tur => tur.other))
      .map(tup => TimeUsageRow(tup._1._1, tup._1._2, tup._1._3, tup._2, tup._3, tup._4))
  ...
  def timeUsageGroupedSqlQuery(viewName: String): String =
    "select working, sex, age, round(avg(primaryNeeds), 1) primaryNeeds, round(avg(work), 1) work, round(avg(other), 1) other " +
      s"from $viewName " +
