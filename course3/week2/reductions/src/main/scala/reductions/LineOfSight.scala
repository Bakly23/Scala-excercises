...
object LineOfSightRunner {
  ...
  def main(args: Array[String]) {
    ...
    val partime = standardConfig measure {
      LineOfSight.parLineOfSight(input, output, 1000000)
    }
    println(s"parallel time: $partime ms")
    println(s"speedup: ${seqtime / partime}")*/


    LineOfSight.parLineOfSight(input, output, 4)
    println(util.Arrays.toString(output))
  }
}
...
object LineOfSight {
  ...
  def downsweep(input: Array[Float], output: Array[Float], startingAngle: Float, tree: Tree): Unit =
    tree match {
      case Leaf(from, until, prevoiusMax) => downsweepSequential(input, output, startingAngle, from, until)
