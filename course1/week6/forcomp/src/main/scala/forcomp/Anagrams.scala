...
object Anagrams {
  ...
  def sentenceOccurrences(s: Sentence): Occurrences = wordOccurrences(if (s == Nil) "" else s.reduceRight(_ + _))
  ...
  def main(args: Array[String]): Unit = {
    println(sentenceAnagrams(List("linux", "rules")))
    println(Some(Nil))
  }
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    if (occurrences.isEmpty) {
      ...
      val occHead = occurrences.head
      val combos = combinations(occurrences.tail)
      (for {
        combo <- combos
        tupleOption <- (1 to occHead._2).map(i1 => (occHead._1, i1))
      } yield tupleOption :: combo) ++ combos
    }
  }
}